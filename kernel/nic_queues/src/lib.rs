...
use memory_model::{MappedPages, create_contiguous_mapping, EntryFlags};
...
pub const NIC_MAPPING_FLAGS: EntryFlags = EntryFlags::default();
...
impl<S: TxQueueRegisters, T: TxDescriptor> TxQueue<S,T> {
    ...
    #[requires(self.num_tx_descs > 0)]
    #[ensures(self.tx_cur == (old(self.tx_cur) + 1) % self.num_tx_descs)]
    // #[ensures(self.regs.get_tdt() == self.tx_cur as u32)]
